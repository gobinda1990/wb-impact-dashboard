    ...
    <Container fluid className="mt-4 px-3 home-container">
      <div className="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 className="text-primary fw-bold mb-2 d-flex align-items-center">
          <FaProjectDiagram className="me-2" /> Dashboard
        </h2>
      </div>
      ...
      {loading ? (
        ...
        <Alert variant="info" className="text-center shadow-sm bg-light border-0 py-4 fs-6">
          <FaInfoCircle className="me-2 text-primary" />
          {message}
        </Alert>
      ) : filtered.length === 0 ? (
        <Alert variant="secondary" className="text-center shadow-sm border-0 py-4 fs-6">
        ...
        <Row className="g-4">
          {filtered.map((proj) => (
            <Col key={proj.projectId} xs={12} sm={6} md={4} lg={3}>
              <Card className="project-card h-100 border-0 shadow-sm fade-in">
                <div className="card-accent" />
                <Card.Body className="d-flex flex-column justify-content-between">
                  <div>
                    <Card.Title className="fw-semibold text-primary d-flex align-items-center mb-2" title={proj.projectName}>
                      <FaProjectDiagram className="me-2" /> {proj.projectName}
                    </Card.Title>
                    <Card.Text
                      className="text-muted small mb-3"
                      title={proj.projectDesc || "Government service portal"}
                    >
                      {proj.projectDesc?.length > 60
                        ? proj.projectDesc.slice(0, 60) + "..."
                  ...
                  <div className="d-flex justify-content-between align-items-center mt-auto flex-wrap gap-2">
                    <span className="badge bg-light text-primary border py-1 px-2">
                      ID: {proj.projectId}
                    </span>
                    <Button
